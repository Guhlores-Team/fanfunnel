...
npm run e2e:supabase   # PROVES isolation against a live (TEST) Supabase project
```
...
- **Runtime test** (`e2e/supabase.mjs`, run via `npm run e2e:supabase`) — creates
  two throwaway creators, signs in as each with the anon key, and asserts creator
  A reads/updates/deletes **zero** of creator B's rows across fans, wheels,
  prizes, spins, grants, and profiles. Run it after any RLS migration against a
  dedicated TEST project (it skips cleanly when the Supabase env vars are unset).
...
1. Run `npm run e2e:supabase` against a TEST project — it will fail loudly and name the table.
