...
- [ ] **Admin bootstrap** — a one-time, env-guarded "claim admin" so the owner
      becomes admin without touching the DB. Fixes `/admin` access (#4).
- [ ] **Org layer** (migration 0014): `orgs`, `org_members(role)`,
      `org_member_creators`, `profiles.org_id`. `org_role` enum.
- [ ] **`can_act_for(creator_id, perm)`** SECURITY DEFINER predicate; split the
      `for all` owner RLS policies per-command so seats get scoped powers.
- [ ] **`org_account_stats(org_id)`** (generalize `admin_account_stats`) → org
      roll-up (revenue + fans + spins + pending across the org's creators).
- [ ] **Agency dashboard** at `/admin` (or `/agency`): roster, roll-up, switch
      into a creator, manage seats + permissions.

## Phase 8B — Autopilot "Today" tab (prescriptive action feed)
- [ ] **`getAutopilot()`** — generates ranked action cards from existing getters
...
- [ ] **12 card types** (DM whales, win-back, hot-slot drop, over-given rarity,
      restock, overdue fulfil, answer fans, out-of-spins, campaign fuel/leak,
      set costs, wishlist spike).
- [ ] **"Today" tab** — ranked cards, each with one-tap execute (reuse DM
...
- [ ] **Prize Book** — collection of won vs. unwon prizes (progressive-disclosure).
- [ ] **Recent-wins ticker** — handle-only opt-in social proof.
- [ ] **Real stock counts** — "3 of 5 left" only when `stock` set.
- [ ] **Creator note / mini-hero** — avatar + one personal line (migration 0016:
      `profiles.avatar_url`, `creator_note`).
- [ ] **PWA** — dynamic per-creator `manifest`, add-to-home nudge after a win.
- [ ] **A11y pass** — `aria-live` spin result, reduced-motion guards, focus traps.
